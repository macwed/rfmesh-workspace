...
post-WS-CD-002 state; ended with full fusion pipeline implemented
...
### Sprint A — fusion pipeline (load-bearing chain)
...
| `1adc42a` | WS-CD-003 Gauss-Newton MLE | bias 17 m → 3 m on 3 km/5°/3-node geometry; **builder caught ticket Jacobian residual-sign bug** |
| `398187b` | WS-CD-004 covariance + 95% ellipse | trench-demo Beat D 358.76 m semi-major matches `crlb_analysis.py` to 0.5 m; **10000-sample inclusion test landed exactly 95 %** |
| `4ad630b` | WS-CD-005 GDOP (ADR-007 D4 unweighted) | Beat C 1.16, Beat D 1.02 |
| `dbd067a` | WS-CD-006 residuals + `is_outlier` flag | strict `> 3·σ`; modulo-form wrap avoid trig round-off at threshold |
...
|---|---|---|
| `a484efa` | `SCHEMA_VERSION 1.1.0` — `L2_CAPON` enum split + ADR-008 + WS-B-007 ticket | additive MINOR bump; WS-B-004 Capon estimator emits `L2_CAPON` (was `L2_MVDR_NULL`) |
...
| `f8d4d0d` | WS-B-006 ticket draft (bundled into architect commit by pre-commit-hook scoop) | threat-library enrichment spec |
...
After 2026-05-17 checkpoint (`90ab700`), lead-Opus ratified four
open decisions in `docs/design/ops-architecture.md` and spawned four
general-purpose builders in parallel against main checkout (no
...
| `8c76cb8` | WS-CD-ops dashboard | 31 tests, 9 panels, 3 layouts (TRENCH / DEBUG / MINIMAL). FixPanel renders %-of-range to **1 decimal**, `n/a` when origin unknown. NullSteeringPanel **caps quoted depth at 20 dB** per ADR-008 §D8. ClassificationOverlayPanel: `None` and `UNKNOWN` both render `"UNKNOWN"`; tooltip differentiates ("no L3 capability" vs "classifier ran, unsure"). ResidualsPanel **re-derives `is_outlier`** per ADR-005 §D3 (contract `FixEvent.residuals_deg` is bare `tuple[float,...]`; flag does not cross wire). Added one `ignore_imports` line for `rfmesh_ops.panels.null_steering -> rfmesh_dsp.l2_null_steering` so `independence` clause accepts D2 carve-out; companion `forbidden` clause still pins every other `rfmesh_dsp` submodule. |
| `9b08096` | WS-CD-cot PyTAK publisher | 45 tests across 4 files + byte-exact `canonical_fix.xml` golden. **WGS-84 meridional/transverse curvatures** in `ellipse_to_polygon_vertices` (not naive equirectangular) — sub-metre accuracy at demo ranges, <1 m at lat 70°. Stale time = `fix.t_unix_ns + 30 s` (not `now + 30s`) — replays stay honest. Transport errors → `CotTransportError`, no auto-reconnect (B3). `range_m=0.0` placeholder in remarks; ops owns operational % display per ADR-005 D5b. Added `cryptography>=42` runtime dep because PyTAK 6.4.0 has buggy `warnings.warn(exc)` when cryptography absent (upstream bug; lifted locally to keep demo immune). |
| `f8b2a48` | WS-CD-node composition root | 39 tests, 14 source files. **Subscriber-registration race regression test landed** (`test_fusion_service.py::test_subscriber_registration_race`, per `INHERITED_CONTEXT.md` §5.2) — design prevents race structurally (inbound queue independent of dashboard pubsub list). **Bearer Protocol is synchronous** (builder caught architect-doc vs contract conflict — contract wins per B1; WifiBearer uses non-blocking socket). LoraBearer ships `loopback=True` v1.0; real-serial path raises `NotImplementedError` (B3 honesty); BothBearer suppresses LoRa `NotImplementedError` so Wi-Fi keeps shipping. L2-uncalibrated → `CapabilityMismatchError` at startup (raise, not exclude). |

**Honest-engineering deviations (documented, no contract change):**

- WS-CD-ops `is_outlier` re-derivation. Contract carries
  `residuals_deg: tuple[float, ...]` (per `FixEvent`); outlier flag
  internal to `rfmesh_fusion.residuals.ResidualsResult`, not
  on wire. Re-deriving in panel from live
  `BearingReport.azimuth_sigma_deg` stream is honest path. Builder
  documented in `packages/rfmesh-ops/src/rfmesh_ops/panels/residuals.py`
  + README.
- WS-CD-ops added `pytest-asyncio>=0.23` to workspace dev deps. Needed
  for dashboard WebSocket round-trip + dashboard.run async tests.
- WS-CD-node caught workspace-root `pyproject.toml` bug:
  `per-file-ignores` glob `tests/**` was root-anchored, no
  effect on package tests (`packages/<pkg>/tests/**`). Fixed to
  `**/tests/**`. Carries `S` + `ANN` + `PLR2004` + `PLC0415` for tests
  — workspace-wide.
- WS-CD-node delivered `Bearer` Protocol with **sync send/recv** to
  honor contract in `rfmesh_contracts.protocols.Bearer`.
  Architect doc had `async def` on same methods; conflict —
...
- WS-CD-node uses **lazy `if TYPE_CHECKING:` imports** for `rfmesh-cot`
  ...
  `ImportError` so fusion-server CLI runs headless even when those
  packages have not landed yet.
- WS-CD-cot added `cryptography>=42` to lift PyTAK 6.4.0 upstream
  bug (`warnings.warn(exc)` passes Exception instead of str/Warning,
  raises `TypeError` on import when cryptography absent — official
  PyPI install includes it transitively, uv resolver does not).

### Post-batch council audit + BLOCK fix + demo-replay

After 4-builder batch landed (`0dbaaeb`), council protocol per
`CLAUDE.md` lines 23-49 ran (architect / code-reviewer /
...
| Demo-integrity | APPROVE | 7 RECOMMENDATIONs. R1 (CoT `range_m=0.0` cosmetic dishonesty) + R4 (`NodeStatusPanel` missing `status_detail` column) reinforced architect BLOCK. |
| Architect | **BLOCK on `f8b2a48`** | B3 violation: `BothBearer` silently suppressed `LoraBearer.NotImplementedError` → `Node._build_status` hard-coded `healthy=True, status_detail=""` → `NodeStatusPanel` shows green when redundancy broken. Operator-invisible failure mode in most jury-visible diagnostic channel. |
...
4 new regression tests (2 bearer-level + 2 heartbeat-level). All gates green: 43 node + 45 cot + 31 ops; mypy strict + ruff + lint-imports (6 KEPT). Architect re-audit predicate flips: 4× APPROVE achievable next pass.

### apps/demo-replay (`d3baabc`)

Greenfield app per architect §2.4 + §3.3. 20 tests (5 scenario loader + 5 ReplayMetadata + 3 ReplayRecorder + 5 ReplayOrchestrator + 2 CLI/aux). In-process orchestration for v1.0 (multi-process is documented migration path; orchestrator seams `_NodeContext` + shared `FusionService`/`DashboardPubSub` already match multi-process shape).

- `SyntheticReceiver` replay-mode extension **deferred to WS-A-005** — `.iqx + .json` format + `ReplayMetadata` ship now so Phase-C bench captures land in stable format; `NodeReplaySpec.replay_iqx_path` field in place; actual playback path is WS-A-005 follow-up.
- `ReplayMetadata` extends actual `IQMetadata` field names (`n_samples`, `start_time_utc`, `format` — not brief's guessed `total_samples`/`started_at_utc`/`dtype`).
...
Closes non-blocking follow-up list surfaced by four-reviewer council audit; no contracts touched, `SCHEMA_VERSION` unchanged. 15 files, +265/-27 LoC across cot/dsp/ml/node/ops + pyproject + ADR-008 + 1 new parking-lot ticket.
...
`uv run pytest -m "not hardware"` workspace-wide at six-commit
landing: **474 passed**. Composition: 335 pre-batch + 138 from 4-
builder batch + 4 BLOCK-fix regression + 20 from demo-replay - 23 overlap = 474.

### Earlier integration check (4-builder batch only)

(Historical, before architect BLOCK fix.)

`uv run pytest -m "not hardware"` workspace-wide at four-builder
landing: **450 passed in 157.33 s** (from 335 pre-batch). +115 tests:
B-006 (23 new) + ops (31) + node (39) + cot (45) = 138 new minus 23
overlap counted in B-006 prior 24-test baseline.

Workspace state after batch:
- 6 lint-imports contracts KEPT, 0 broken (D1 + D2 + WS-B-005
...
- mypy strict: every package green at own scope; workspace
  `[tool.mypy.exclude]` keeps tests out as before. Post-checkpoint
  `**/tests/**` per-file-ignores fix is only non-package edit to
  `pyproject.toml` from batch.
- ruff check + format: every package green.

**Builder agent type lesson learned (operational, not architectural):**
First spawn attempt used `caveman:cavecrew-builder` for all four
...
main checkout (its `--isolation=worktree` flag honored in name
only — file edits used absolute paths to main) before killed.
General-purpose agents are right choice for greenfield package
...
All four scenarios inside [0.92, 0.98] pass band. **Pipeline (L1 / L2 σ honesty → Stansfield → Gauss-Newton MLE → Fisher-info Σ → χ²-scaled ellipse → ConfidenceLevel band) empirically honest at 95 % inclusion level.** Frobenius ratio (empirical Σ vs claimed Σ): 1.146 / 1.020 / 1.002 / 1.030 — all dead-centre at 1.0. Mean position bias < 0.5 × semi_minor on every scenario.

## Demo narrative — binding facts

Per `docs/demo/script.md` + `docs/adr/ADR-009-*.md`:

- Trench-demo lives in **MEDIUM** band on all four beats. HIGH only at denser deployment than 4-node demo. Honest pitch: "percentage shrinks from 49 % → 26 % → 32 % as nodes join" — *shrinking* is deployment-density story.
...
Single-day procedure, no servo required. Awaiting Maciej first
execution (planned for morning after this sprint). PASS = peak
within ±10° of true bearing, ≥ 6 dB prominence. FAIL routes to one of
5 pre-enumerated diagnostic branches from `INHERITED_CONTEXT.md`
§3.1.1.

If Phase C reveals real-world σ inflated > 1.5× over simulator,
ADR-005 5 % threshold may need re-tuning (single-line constant, ADR-005
§D6 anticipates this); simulator `MultipathFIRChannel` tap
configuration retunes accordingly.

## Open lead decisions for ratification

Surfaced by architect design doc (`docs/design/ops-architecture.md`)
+ WS-B-006 drafter notes. Each small, all four can be ratified
in one short pass:

1. **Import-linter relaxation for `rfmesh-node`** (architect §4.1).
   `rfmesh-node` is single composition root; cannot satisfy Star
   `independence` contract by definition. Carve-out: relax `Star`
...
2. **`rfmesh-ops` carve-out for `rfmesh_dsp.l2_null_steering`**
   ...
   from `rfmesh-dsp`. Add named exception to lint-imports.
...
4. **`pyyaml` runtime dep** (WS-B-006 drafter notes). Needed for
   threat-profile YAML loader. ~few KB pure-Python. Approve
   `/uvadd-request` to unblock WS-B-006 build.

Lead recommendation: **accept all four** with documented
justifications. Council can revisit if real-data testing surfaces
issue.

## Three `[needs validation — TBD]` flags

From demo-integrity agent `docs/demo/script.md`:

1. **Precise on-day null-depth number** — waits on recorded-IQ buffer captured against canonical 2-emitter scenario. Likely lands at 15-20 dB per WS-B-007 MC.
2. **Pre-rendered slide-deck fallback** for demo §5 step 4 — content TBD when slide deck itself exists.
3. **Non-FTS C2 stack integration** — for §6 honesty section. Current PyTAK adapter targets FreeTAKServer; behaviour with other CoT servers undocumented.
...
Slow-marker MC suite (excluded from default verify): `uv run pytest -m slow` runs 4 honest-ellipse scenarios in ~3-4 s on commit `75a6f82`.
...
- Conversation messages: 308 k (bulk).
...
Ran `rfmesh-demo-replay` orchestrator end-to-end against simulator while Maciej executed Phase C on bench. Artifacts under `docs/demo/artifacts/`.
...
**Honest finding (Phase-C-relevant):** design-intent scenario `trench_demo.yaml` produces **zero L1 bearings** at documented antenna heights (tx 3 m / rx 2 m) over 2.2 km at 915 MHz. Two-ray destructive null suppresses on-axis RSSI to ~5 dB above off-axis floor; L1 prominence gate (6 dB default) correctly refuses (Invariant B3). This is multipath-dominance failure mode pre-enumerated in `INHERITED_CONTEXT.md` §3.1.1 — simulator faithfully models it.
...
- **NEXT-2** — add `--channel-override` flag to `rfmesh-demo-replay` so artifact capture does not need sister YAML.
- **NEXT-3** — fix `--headless` mode: orchestrator `_relay_fixes_to_sink` requires dashboard queue to also exist. Trace `apps/demo-replay/src/rfmesh_demo_replay/replay.py:545`.

**Sister scenario:** `scenarios/trench_demo_artifact.yaml` (free-space channel, same geometry/nodes/beats as `trench_demo.yaml`), only for artifact capture — does not replace design-intent file.

**Capture harness:** `scripts/capture_demo_artifacts.py` — programmatic orchestrator + subscriber + per-beat dashboard render, bypasses `--headless` bug. Reproducible: `uv run python scripts/capture_demo_artifacts.py`.
...
Ellipse axes simulator-clean (order of magnitude tighter than Monte-Carlo multipath-loaded numbers). Pipeline integrity confirmed; honesty budget unchanged.
---

## 2026-05-17 late evening — Tier B follow-ups

Parallel to Phase C on bench. Four medium-value tasks.

### B1 — 105619f row added to SPRINT_LOG (this section's sister)

15 council follow-ups bucket-sorted into doc / RF-DSP NOTES / Demo-integrity R / test fixes / parking-lot. See "15 non-blocking council follow-ups (`105619f`)" section above.

### B2 — memory compaction

- `INHERITED_CONTEXT.md`: 20109 → 17962 bytes (~11 % reduction); backup `INHERITED_CONTEXT.original.md`.
- `docs/SPRINT_LOG.md`: 24775 → 24296 bytes (~2 % reduction); backup `SPRINT_LOG.original.md`. The SPRINT_LOG is already lean (tables + commit refs); little compression headroom.

### B3 — hypothesis property tests for fusion sigma-honesty

`packages/rfmesh-fusion/tests/test_property_invariants.py` — 6 tests, hypothesis budgets ~12-20 examples each:

| Test | Invariant |
|---|---|
| `test_sigma_scaling_invariance` | Multiplying every sigma by k scales ellipse semi-axes by k (rel 2 %); GDOP unchanged. |
| `test_permutation_invariance` | Reordering bearings preserves position, ellipse, GDOP; residual multiset preserved; contributing_nodes reorders with input. |
| `test_translation_invariance` | Rigid shift of nodes + emitter preserves ellipse and GDOP (rel 0.5 %). |
| `test_residuals_zero_sum_no_outlier_under_clean_geometry` | Exact-azimuth input: no residual flagged as outlier (|r| < 3*sigma). |
| `test_unweighted_gdop_independent_of_sigma_choice` | GDOP identical between sigma=0.5 and sigma=8 (ADR-007 D4). |
| `test_confidence_level_monotonic_under_sigma_inflation` | Inflating sigma cannot move `confidence_level` toward HIGH. |

All pass in 0.18 s. No invariant violated → fusion math honest under these axes.

### B4 — TBD #1 pre-computed (on-day null-depth number)

`scripts/null_depth_mc_stats.py` runs the same 1000-trial robust MC as `test_null_depth_robust_under_mismatch` (UCA-4, ±2° steering mismatch + ±10° per-channel calibration error). Headline:
...
| p5 | 36.42 dB |
| p95 | 62.62 dB |
| 90 % CI of mean | [46.91, 47.74] dB |

**Far above the ADR-008 §D8 ≤ 20 dB UI cap.** Honest interpretation: the simulator does not model residual error sources a real bench-side capture carries (analog I/Q imbalance, narrow-band fading, residual cable phase drift beyond ±10°, frequency-selective array element mismatch). Real-world numbers will fall below the simulator median; the **≤ 20 dB UI cap holds**, and the "−18 dB" slide caption stays.

Read-out lands in `docs/demo/null_depth_mc_stats.md`. `docs/demo/script.md` Beat E.2 + §6 TBD list updated to point at it.

### Aggregate Tier-B verification
